...
class Ajax_pagination{
	...
	var $last_link		= 'Akhir';
	var $uri_segment	= 3;
	var $full_tag_open	= '<ul class="pagination pagination-sm float-right pb-0 mb-0">';
	...
	function create_links(){
		...
		if ($num_pages == 1){
            		$info = 'Menampilkan : ' . $this->total_rows.' data';
			return $info;
		}
		...
		if ($this->show_count){
			...
			$info = '<li class="page-item"><a class="page-link">Menampilkan ' . ( $curr_offset + 1 ) . ' sampai ' ;
			...
			$info .= ' dari ' . $this->total_rows . ' data | </a></li>';
