...
<script>
    ...
    const tampilAwal = () => {
        $.ajax({
            url: '<?= base_url() ?>kembalipesrom/getdata',
            method: 'post',
    ...
    const saveData = id => {
        ...
        $.ajax({
            url: '<?= base_url() ?>kembalipesrom/save',
            method: 'post',
            data: {
                id,
                kembali
    ...
    tampilData = id => {
        $.ajax({
            url: '<?= base_url() ?>kembalipesrom/getid',
            method: 'post',
            data: {
                id
            },
    ...
    $('#tampil').on('click', '#batalSurat', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '<?= base_url() ?>kembalipesrom/batal',
                    method: 'post',
                    data: {
                        id
                    },
    ...
    $('#idsantriijin').on('keyup', function(e) {
        ...
        if (id != '' && e.keyCode == 13) {
            $.ajax({
                url: '<?= base_url() ?>kembalipesrom/cekmodal',
                method: 'post',
                data: {
                    id
                },
    ...
    const dataModal = id => {
        $.ajax({
            url: '<?= base_url() ?>kembalipesrom/getmodal',
            method: 'post',
            data: {
                id
            },
    ...
    $('#simpanijin').on('click', function() {
        ...
        if (alasan == '') {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '<?= base_url() ?>kembalipesrom/saveijin',
                        method: 'post',
                        data: {
                            id,
                            alasan
    ...
    $('#tampilData').on('click', '.checked', function() {
        $('#filter-type').val(1)
        $('#title-detail').text('Daftar Santri Kembali Pesrom')
        showmodalfilter()
    })

    $('#tampilData').on('click', '.not-checked', function() {
        $('#filter-type').val(0)
        $('#title-detail').text('Daftar Santri Belum Kembali Pesrom')
        showmodalfilter()
    })
    ...
    const showmodalfilter = () => {
        ...
        $.ajax({
            url: '<?= base_url() ?>kembalipesrom/showfilter',
            method: 'post',
            data: {
                type,
                domicile
