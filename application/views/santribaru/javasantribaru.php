...
<script>
    ...
    $('#niksantri').on('focusout', function() {
        let isi = $(this).val()
        if (isi === '') {
            return false
        }
        ...
        $.ajax({
            url: '<?= base_url() ?>santribaru/cekniksantri',
            method: 'post',
            data: {
                isi
            },
            ...
            success: function(data) {
                ...
                Swal.fire(
                    'NIK SUDAH ADA',
                    'Anda tidak bisa input data dengan NIK yang sama',
                    'error'
                )
            }
        })
    ...
    const pesangagal = $('#pesangagal').data('id')
    if (pesangagal != '') {
        toastr.error(pesangagal)
    }
